          ...
          let iconsAdd = '';
            ...
            if(platform === 'MOTUM'){
              ...
              if(platform === 'CUMMINS'){
                ...
                if(platform === 'ENLACE FREIGHTLINER'){
                  iconsAdd += `<div class="appCircle platform" title ="icon">
...
onClickMoreApps(targetElement){
  ...
  getClientsData(){
      ...
      //         }
      //     );
      // const params = {clientId: 1, userTypeId: 1};
      const params = [{'name': 'clientId', 'value': 1},{'name': 'userTypeId', 'value': 1}];
      this.userService.getDataUsers(params)
        .subscribe(
          res => {
            const body = JSON.parse(res['_body']);
           
            if(this.gridOptions.api && body){
