...
import { RequestOptions, Http, Headers, ResponseContentType, URLSearchParams } from '@angular/http';
 ...
 // create new user
  ...
  // edit user
   ...
   editUserMembersResponse$ = this.ShowEditMember.asObservable();
    ...
    constructor(private api: ApiCrudService, private C: Constants, private http: Http) {}
    ...
    getDataUsers(params?: any):Observable<Response>{
      let shareGet;
      let url = 'http://users-api-dot-socketavl.appspot.com/users';
      if (params) {
        ...
        params.forEach(param => {p.set(param['name'], param['value'])});
        
        let options = new RequestOptions({headers: headers, params: p});

        shareGet = this.http.get(url, options);
      }
